...
| `…/submissions/from-invoice` | `POST` | `submit` | Resolve FA(3) from an issued sales invoice `{salesInvoiceId}`; `202`. 404 unknown / 409 not-issued·proforma·no-credentials / 422 doc-type·seller·buyer·self-billing·issue-date. |
...
### Verdict
**Compliant** — implemented, gate-passing, and live-verified on KSeF TEST for the online send + inbound receive paths (token auth). Cert/XAdES auth and KOD II are unit-proven with env-gated live blocks ready; the JPK→MF submit and batch round-trips are client-unit-verified with recipes ready (not live-exercised at time of consolidation — see the sibling SPEC-006 for the JPK gateway caveat).
...
- **Not live-exercised** (documented, env-gated recipe ready): the JPK→MF submit round-trip (SPEC-006) and the batch session round-trip — both client-unit-verified. The cert/XAdES auth path and KOD II are unit-proven; the first cert must be enrolled externally with a qualified signature.
