...
- **2026-06-30 (IMPLEMENTED).** Built across coordinated subagents; full gate + 4-model code jury (2 rounds) + sandbox preview on released core.
  - **Code jury round 1** (Claude `request_changes`; Codex/Kimi/DeepSeek `fail`) — fixed: invoice-meta server-side lock (the custom route never ran the interceptor → fail-open), DELETE `?id=` interceptor bypass, CorrectionForm wrong response field + negative quantity + missing currencyCode/issueDate, `orderSnapshot` added to the PL-VAT editor, edit-form flat→InvoiceMeta meta mapping + reactive currency, inverted `hasAllFeatures` on the certificates page; tests strengthened (TC-UI-002 meta+DELETE 409; TC-UI-004 real correction payload).
  - **Code jury round 2** (Claude `pass`, DeepSeek `pass`; Kimi/Codex 3 each) — fixed: `kodUrzedu` (+ optional contextNip) added to the JPK generate UI (resolver requires it), Issue-offline gated on `financial_pl.manage` (matches the route), invoice-meta PUT composed gate `[financial_pl.manage, sales.invoices.manage]`, CorrectionForm currency defaults to PLN. Non-blocker: Kimi's optimistic-lock claim (the helper is additive — no header ⇒ no enforcement, so editing does not fail).
  - **Sandbox preview (released core, DB `om_fpl_spec013`):** "Financials (PL)" nav + Invoices list/create/detail + JPK + Certificates render; create→list→detail verified live; KSeF status badge + numer KSeF; Edit locked when accepted; KOR form with positive quantities. **Preview-found + fixed:** the backend `/[...slug]` catch-all passes `params` as a sync prop, so detail/edit pages must read `props.params.id` — `useParams()` returned the slug array (empty id → 308 → "Failed to load"). Reverted to the sync prop (mirrors core `sales/.../[id]/page.tsx`).
  - **Gate:** build:packages PASS · generate PASS · module jest 377 passed/10 skipped · i18n:check-sync PASS · our-source typecheck 0 errors. Pre-existing platform gaps (NOT this feature; also break the merged `forms` module / the whole sandbox): vendored `@open-mercato/ui` `DataTable.tsx` type errors; `build:app` needs `@mdxeditor/editor` + `@radix-ui/react-scroll-area` (core UI/content deps missing from the stale install — installed locally to run the preview).
