...
### Adapter registration for Phases 4–5 (shell adapters)
`magentoInventoryAdapter` and `magentoOrdersAdapter` remain **registered shells**: each implements `getMapping`/`validateConnection` (per this spec's Phase 4–5 sketches) but **no `streamExport`/`streamImport`** yet. This makes their Sync Schedule tabs functional (mapping preview, connection validation) without enabling actual data transfer. Full implementations remain pending the addendum specs called out in Phases 4–5.
