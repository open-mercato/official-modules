...
- **`magento_products`** (export, `catalog.product`): simple OM products → Magento simple products, with dynamic EAV attribute + category provisioning. **Implemented** (see Implementation Status).
- **`magento_prices`** (export, `catalog.product_price`): base + special + tier prices → Magento bulk price endpoints. **Implemented** (see Implementation Status) — payload shapes, connectivity, channel mapping, and a clean end-to-end run (6/6 success) all confirmed against a live Magento sandbox; Review Gate closed.
- **`magento_inventory`** (export, `catalog.product_stock`): per-channel stock quantities (sourced from a configurable custom field) → Magento MSI source items. *Architecture-level sketch — deliberately deferred, see Implementation Status.*
- **`magento_orders`** (import, `sales.order`): Magento orders + customer/address data → `sales_orders` (+ `customer_entities` via command). **Implemented** (see Implementation Status) — live-validated against a real Magento sandbox (order import, customer create/link dedup, idempotent re-run all confirmed).
...
> **Deferred (2026-07-14)**: this phase's `stock_qty_<channelCode>` custom-field
> data source (below) is superseded by the new `wms` core module (Warehouse/
> InventoryBalance/InventoryReservation — see
> [open-mercato/open-mercato#1701](https://github.com/open-mercato/open-mercato/pull/1701)),
> which is the real, durable stock source of truth this adapter should read
> from once it lands. Implementing Phase 4 against the custom-field convention
> now would mean re-doing it against `wms` shortly after — not started. Revisit
> once WMS Phase 1 (or a stock-read API built on it) is available.
...
## Phase 5: `magento_orders` (import, `sales.order`) — Implemented

See Implementation Status below for what shipped and how it was live-validated; the sketch below is kept for the original design rationale.
...
- **Addendum resolved during implementation (2026-07-14)**: `sales.orders.create`/`customers.people.create` input shapes confirmed against current `@open-mercato/core` (see Implementation Status — notably `orderCreateSchema` recomputes totals from `lines` server-side rather than accepting Magento's totals verbatim, and `customerSnapshot` must nest under a `customer` key, not a flat record). No dedicated Magento search-criteria builder existed; reused the existing flat bracket-notation `query` pattern already established in `lib/adapters/attributes.ts` rather than adding one to `client.ts`.
...
- [x] `PUT /rest/V1/products/{sku}` and `GET /store/storeViews` payload shapes confirmed against a live Magento 2.4 sandbox (2026-07-09): requests reach Magento and response shapes match what `lib/store-views.ts` and `lib/adapters/prices.ts` assume. `/products/attributes`, `attribute-sets/sets/list`, `attribute-sets/{id}/attributes`, `/products/{sku}/media` were not separately exercised in this session.
- [ ] **Known limitation, not a defect**: live validation surfaced that `exportConfigurableProduct` silently skips setting per-variant configurable attributes when the parent has no `option_schema_id`, causing Magento to reject the product ("same set of attribute values"). Root-caused to a seed-data gap in the sibling `open-mercato` repo (`packages/core/src/modules/catalog/seed/examples.ts` creates variants with populated `optionValues` via `em.create()` directly, bypassing the command/API layer that would normally link an option schema) — the real UI/API cannot produce this state, so no fix is required here. Suggested (optional) hardening: have `products.ts` warn/skip instead of silently omitting attributes when a multi-variant configurable has no linked option schema, so a future occurrence fails loudly instead of producing an invalid Magento payload.

### Adapter registration for Phase 4 (shell adapter, deferred)
`magentoInventoryAdapter` remains a **registered shell**: it implements `getMapping`/`validateConnection` (per this spec's Phase 4 sketch) but **no `streamExport`** yet. This makes its Sync Schedule tab functional (mapping preview, connection validation) without enabling actual data transfer. Deliberately not implemented — see the Phase 4 deferral note above (superseded by the `wms` core module).

### Phase 3: `magento_prices` (export) — Implemented
- `lib/store-views.ts` (new) — `fetchStoreViewsByCode(client)` calls `GET /rest/V1/store/storeViews` once per run and indexes the result by `code`, resolving `MagentoSyncSettings.channelStoreMappings[].storeViewCode` to the numeric `store_id`/`website_id` the bulk price endpoints require.
- `lib/adapters/prices.ts` — `streamExport` implemented (previously a shell with only `getMapping`/`validateConnection`). Per run: pages `CatalogProductPrice` rows (not `CatalogProduct` rows — see cursor note below), resolves each changed row's target (a configurable product's variant, or a variant-less simple product), reloads that target's *full* current price picture, then batches base/special/tier entries per configured channel (or once globally if no `channelStoreMappings` are set) into three Magento bulk calls: `POST /products/base-prices`, `POST /products/special-price`, `POST /products/tier-prices`.
- Channel resolution: for each `channelStoreMappings` entry, prefers a price row scoped to that `channelId`, falling back to the channel-agnostic (`channelId: null`) row — same convention as the `magento_products` adapter's own price resolution. With no channel mappings configured, prices go to Magento's global scope (`store_id: 0` / `website_id: 0`). Tier prices are deduped per `(website_id, minQuantity)` since Magento tier prices are website-scoped, not store-view-scoped.
- **Extends the original sketch**: the Phase 3 sketch (written 2026-06-12) predates configurable-product support (added 2026-06-16) and only covered top-level `CatalogProduct` rows. The implementation covers **both** standalone simple products and configurable products' variants (each variant is its own priced Magento simple SKU), since the "fast price sync" path should reach every SKU `magento_products` can create in Magento, not just top-level product rows. Price rows attached directly to a configurable *parent* are skipped (configurables have no own settable price in Magento).
- **Cursor**: keyed on `CatalogProductPrice.updatedAt`/`id` (the price row itself), not the product's `updatedAt` as Phase 2 uses — so a price-only change (e.g. only a special price row touched) advances the cursor and gets picked up on the next incremental run, per the spec's cursor note.
- Bulk endpoint responses only list *failed* items (`{ message, parameters }`, no dedicated SKU field); failures are attributed back to a SKU via a best-effort match against `parameters`/`message`. Unmatched failures are logged (`console.error`) but can't be attributed to a specific item — see Review Gate below.
- New event `sync_magento.price.pushed` added to `events.ts` (emitted per batch with a success count), mirroring the Phase 4 sketch's `sync_magento.inventory.pushed` convention.
- Unit tests added: `__tests__/prices.test.ts` (12 tests: base/special/tier push, channel preference + fallback, missing store view handling, orphaned/configurable-parent price rows, bulk failure attribution by parameters, whole-request failure, no-price-data skip, adapter shape). `yarn workspace @open-mercato/sync-magento build`/`typecheck`/`test` all pass (9 suites, 124 tests).
- Compliance self-review: no `any`, no raw `em.find`, all queries via `findWithDecryption` and scoped by `organizationId`/`tenantId`; the only HTTP call is via `ctx.client` (the existing Magento REST wrapper), no raw `fetch`.

#### Phase 3 Review Gate — residual risk
- [x] `POST /products/base-prices`, `/products/special-price`, `/products/tier-prices`, and `GET /store/storeViews` confirmed reachable against a live Magento 2.4 sandbox (2026-07-09); response shapes match what the adapter assumes. Retires the "payload shapes unvalidated" line from the original review gate.
- [x] Channel-fallback logic (`filterByChannel`) confirmed working as designed once `channelStoreMappings` is configured in Magento Sync Settings — verified against a demo catalog where all seeded price rows were channel-scoped (none channel-agnostic), so with zero mappings configured the adapter correctly skipped all rows (0 pushed, 6 skipped) rather than mis-targeting them. Re-confirmed the mapping (`Mercato Fashion Online` → store view `default`, currency `USD`) persists correctly across a full server restart.
- [x] All 6 demo-catalog SKUs the adapter would submit (`ATLAS-RUN-NAVY-8`, `ATLAS-RUN-GLACIER-10`, `AURORA-ROSE-M`, `AURORA-CELESTIAL-L`, `SERV-HAIR-60-SENIOR`, `SERV-MASSAGE-90-MASTER`) confirmed to exist as real `simple` products in Magento's own `catalog_product_entity` table (queried directly) — rules out "product not found" as the failure cause.
- [x] **Resolved (2026-07-14)**: clean end-to-end run confirmed (6 updated, 0 failed) against the live Magento 2.4 sandbox. Root cause of the prior 6/6 `error` result: `pushPrices()` submitted `store_id`/`website_id` from the resolved channel's store view (e.g. `1`) for base/special/tier prices, but this sandbox's `catalog/price/scope` config is `Global` (Magento's default) — Magento rejects any non-zero store_id for the `price`/`special_price`/tier-price bulk endpoints under Global scope with `"Could not change non global Price when price scope is global"`. Captured the exact rejection by adding a temporary CLI command (`sync_magento debug-prices`, removed after use) that calls `magentoPricesAdapter.streamExport` directly via `yarn mercato`, bypassing the sandbox's unreliable dev-server log mirroring entirely (see `Environment issue` note below for a reusable technique). Fix: added `isGlobalPriceScope(client)` to `lib/store-views.ts` (`GET /products/attributes/price`, whose `scope` field mirrors the `catalog/price/scope` config that also governs `special_price` and tier prices) and threaded it into `resolveChannelTargets()` in `lib/adapters/prices.ts` so every channel target collapses to `store_id`/`website_id` 0 when scope is Global, falling back to the resolved channel's real store/website id otherwise (verified with a new unit test, `submits the channel-resolved store_id when Magento catalog price scope is not Global`). Also deduped base-price entries per resolved `store_id` (`seenBaseStoreIds`) since Global scope can otherwise collapse multiple channel targets into duplicate entries for the same SKU. Verified against Magento's own `catalog_product_entity_decimal` table (store_id 0 rows) that the pushed prices persisted correctly.
- [x] **Resolved (2026-07-14)**: the failure-to-SKU attribution heuristic was exercised against real Magento failure responses during the price-scope debugging above (`special-price` calls failing before the fix) — `parameters: [sku, ...]` reliably included the submitted SKU as the first element in every observed rejection, confirming the best-effort match works in practice on this Magento version. Residual risk (no dedicated SKU field in Magento's response contract) remains theoretical but unexercised failure formats are still possible on other Magento versions/configs.

**Environment issue — resolved 2026-07-09:** `apps/sandbox`'s dev server previously 500'd on every page render with a Turbopack `TurbopackInternalError` (`FileSystemPath("apps/sandbox").join("../../../official-moduler/...")  leaves the filesystem root`). Root-caused to `apps/sandbox/src/app/globals.css`'s Tailwind v4 `@source` directives globbing raw `node_modules/@open-mercato/*/src/**/*.{ts,tsx}` (a pre-existing, already-TODO'd hack), which swept in `__tests__/*.test.ts` files; combined with those packages' catch-all `"./*"` `exports` map, Turbopack's exports-map resolver mis-computed a relative path for a matched test file it was never meant to treat as a module. Fixed by adding `@source not "../../../../node_modules/@open-mercato/*/src/**/__tests__/**";` to exclude test directories from the scan. Verified clean after a full cache clear (`.mercato/next`) and restart — `/login` and `/backend` both render/redirect normally with no Turbopack errors.

### Phase 4: `magento_inventory` (export) — Deferred, not implemented
Deliberately skipped: stock-level sync is superseded by the new `wms` core module ([open-mercato/open-mercato#1701](https://github.com/open-mercato/open-mercato/pull/1701)), which introduces a real inventory core (warehouses, lot-aware stock ledger, balances, reservations) that supersedes this phase's original `stock_qty_<channelCode>` custom-field sketch as the intended stock source of truth. `magentoInventoryAdapter` stays registered as a shell (`getMapping`/`validateConnection` only, per Phase 1). Revisit once WMS Phase 1 (or a stock-read API built on it) is available in `@open-mercato/core`.

### Phase 5: `magento_orders` (import) — Implemented
- `lib/adapters/orders.ts` — `streamImport` implemented (previously a shell with only `getMapping`/`validateConnection`). Per run: pages `GET /rest/V1/orders` via Magento's `searchCriteria` (bracket-notation query params, same pattern as `attributes.ts`), filtered by `updated_at gt <cursor>` and, when `orderImportStatuses` is configured, an ANDed `status in (...)` filter group; sorted `updated_at ASC`.
- Per order: resolves `channelId` from Magento `store_id` → store view code (via `lib/store-views.ts`'s existing `fetchStoreViewsByCode`, reverse-indexed by numeric id) → `channelStoreMappings`, falling back to `defaultOrderChannelId`; `action: 'failed'` with an explanatory `data.errorMessage` when neither resolves (no channel to assign).
- Customer resolution follows `customerStrategy` exactly per the sketch: `skip` never links an entity; `create_or_link` first checks an `ExternalIdMappingService` mapping by Magento `customer_id`, then a `findOneWithDecryption` email match against `CustomerEntity.primaryEmail`, only dispatching `customers.people.create` if neither hits; `create_only` always dispatches `customers.people.create` with no dedup, per spec.
- Line items: each Magento order item's `sku` is best-effort resolved to a local `CatalogProduct.id` via `findOneWithDecryption` (left `undefined`/null when no match — `sales_order_lines.product_id` is nullable exactly for this case); `catalogSnapshot` carries the raw `{sku, name, price}`.
- Dispatches `sales.orders.create` via `commandBus.execute()` (resolved from the DI container, now exposed on `AdapterContext` alongside `em`/`client`/etc. — see `shared.ts`); on success stores the `sync_magento_orders` → `sales.order` external-id mapping and emits `sync_magento.order.imported` (fire-and-forget, `.catch()`-guarded, matching the `product.exported`/`price.pushed` convention). A command failure (validation error, etc.) is caught and yields `action: 'failed'` with `data.errorMessage` — the sync-engine's `logImportItemFailures` picks that key up automatically for the run's log.
- Orders already mapped (re-fetched on a later run, e.g. a status update) yield `action: 'skip'` — per spec, status-sync-on-reimport is out of scope for this MVP (flagged as a future Phase 5b in the original sketch).
- **Addendum findings** (contracts confirmed against current `@open-mercato/core`, differ from the original sketch in two ways): (1) `orderCreateSchema` recomputes order totals server-side from `lines`/`adjustments` via `salesCalculationService` — passing Magento's `grand_total` etc. directly onto the header would be silently overwritten, so only `lines[].unitPriceNet`/`quantity` are populated, not header totals; (2) `customerSnapshot` is `{ customer?: {...}, contact?: unknown }` (passthrough), not a flat record — nested accordingly.
- Unit tests added: `__tests__/orders.test.ts` (15 tests: mapping/validateConnection shape, already-imported skip, no-channel failure, channel resolution via store view mapping, all three `customerStrategy` behaviors including "create_or_link must not create a duplicate when an email match exists" and "create_only must never dedupe", line-item SKU resolution matched/unmatched, command-failure handling, pagination `hasMore`/cursor encoding, status-filter query construction, whole-request failure). `yarn workspace @open-mercato/sync-magento build`/`typecheck`/`test` all pass (10 suites, 140 tests).
- **Live-validated (2026-07-14)** against the real Magento 2.4 sandbox's 2 built-in sample orders, via the same temporary-CLI-command technique used for the Phase 3 price-scope fix (`sync_magento debug-orders`, removed after use — credentials were borrowed in-process from the already-configured `sync_magento_prices` integration since `sync_magento_orders` has none of its own yet in this sandbox, nothing persisted): both orders imported successfully (`action: 'create'`), channel correctly resolved via `channelStoreMappings`, totals recalculated correctly by `salesCalculationService` from the pushed lines (verified equal to Magento's `grand_total` in this case), the second order correctly reused the first order's customer entity (`create_or_link` dedup confirmed against real encrypted data — a raw SQL check by email initially looked like a miss, which turned out to be because `customer_entities.primary_email` is tenant-data-encrypted at rest, not a real bug), and a second run of the same command correctly skipped both orders (`action: 'skip'`, idempotency confirmed).
- Compliance self-review: no `any`, no raw `em.find`/`fetch`, all queries via `findOneWithDecryption` and scoped by `organizationId`/`tenantId`; `commandBus.execute()` calls carry both `organizationId`/`tenantId` in the command `input` and `selectedOrganizationId` in the `CommandRuntimeContext` per the confirmed scope-check contract.
...
### 2026-07-07 (Phase 3: magento_prices export adapter)
- Implemented `magentoPricesAdapter.streamExport`: bulk base/special/tier price push to Magento (`/products/base-prices`, `/products/special-price`, `/products/tier-prices`), channel-aware via `channelStoreMappings` resolved to Magento `store_id`/`website_id` through new `lib/store-views.ts`, falling back to Magento's global scope when no channels are configured.
- Extended beyond the original Phase 3 sketch to also cover configurable products' variants (not just top-level simple products), since those are separately-priced Magento SKUs created by `magento_products`. Cursor now tracks `CatalogProductPrice.updatedAt`/`id` directly so price-only changes sync incrementally without waiting for the parent product/variant row to change.
- Added `sync_magento.price.pushed` event; 12 new tests in `__tests__/prices.test.ts`; all 124 tests pass, `build`/`typecheck` clean.

### 2026-07-09 (live sandbox validation, Phase 3)
- Validated Phase 3 (`magento_prices`) against a real Magento 2.4 sandbox: `GET /store/storeViews`, `PUT /products/{sku}`, and the bulk base/special/tier price endpoints are all reachable and match the payload shapes assumed by `lib/store-views.ts` and `lib/adapters/prices.ts`. Channel-fallback logic confirmed correct.
- Surfaced (but did not fix, out of package scope) a seed-data gap in the sibling `open-mercato` repo that produces configurable products Magento rejects; documented as a non-blocking known limitation with an optional hardening suggestion for `products.ts`.
- Fixed the Turbopack blocker that was preventing a final end-to-end run: root-caused to a Tailwind `@source` glob in `apps/sandbox/src/app/globals.css` sweeping `__tests__/*.test.ts` files from `node_modules/@open-mercato/*`, colliding with those packages' wildcard `exports` map (see Review Gate above for detail). Fixed with an `@source not` exclusion; confirmed `/login` and `/backend` render cleanly after a full cache clear and restart.
- Re-configured the `channelStoreMappings` (`Mercato Fashion Online` → store view `default`, currency `USD`) — the prior session's mapping had not persisted through the earlier crash — and re-ran `sync_magento_prices`. The run now genuinely attempts the push (all 6 SKUs confirmed to exist in Magento) but all 6 come back as `status: 'error'` rather than the previous `skipped`. The specific Magento rejection reason could not be captured in this session due to dev-environment log-visibility limits (see Review Gate) — this is a new, narrower open item, not the same issue as before.

### 2026-07-14 (Phase 3 price-scope fix, Review Gate closed)
- Root-caused and fixed the 6/6 `error` result left open on 2026-07-09: Magento rejected every base/special/tier price push with `"Could not change non global Price when price scope is global"` because `lib/adapters/prices.ts` submitted the resolved channel's numeric `store_id`/`website_id` even though this sandbox's `catalog/price/scope` config is `Global` (Magento's default), which only accepts `0` for those endpoints. Captured the exact Magento error by adding a temporary `sync_magento debug-prices` CLI command that calls `magentoPricesAdapter.streamExport` directly via `yarn mercato` (outside Next.js), avoiding the dev-server's unreliable background-worker log mirroring; removed the command after use.
- Added `isGlobalPriceScope(client)` (`lib/store-views.ts`, `GET /products/attributes/price`) and threaded it through `resolveChannelTargets()` so every channel target uses `store_id`/`website_id` 0 under Global price scope and the real per-channel id otherwise; deduped base-price entries per resolved `store_id` to avoid duplicate submissions when Global scope collapses multiple channels together.
- Verified with a clean end-to-end run against the live sandbox (6 updated, 0 failed) and directly against Magento's `catalog_product_entity_decimal` table (`store_id = 0` rows hold the pushed prices). Added a unit test for the non-Global-scope path (`__tests__/prices.test.ts`); updated one existing test's expected `store_id`. `yarn workspace @open-mercato/sync-magento build`/`typecheck`/`test` pass (124/125 tests — the 1 remaining failure, `preset.test.ts`'s `integrationLogService.scoped` id expectation, predates and is unrelated to this fix, see note below).
- This closes both remaining Phase 3 Review Gate items (clean end-to-end run; failure-to-SKU attribution heuristic exercised against a real rejection).

### 2026-07-14 (preset logging fix; Phase 4 deferred; Phase 5 implemented)
- Fixed `applyMagentoEnvPreset`'s preconfiguration log: it was writing under the bundle id (`'sync_magento'`) instead of a registered integration id, so the log never surfaced on any integration's log tab. Now logs under `MAGENTO_INTEGRATION_IDS[0]` (`sync_magento_products`).
- Deferred Phase 4 (`magento_inventory`): stock sync is superseded by the new `wms` core module ([open-mercato/open-mercato#1701](https://github.com/open-mercato/open-mercato/pull/1701)); not implemented, `magentoInventoryAdapter` stays a registered shell. See the Phase 4 section for detail.
- Implemented Phase 5 (`magento_orders` import adapter): `streamImport` fetches Magento orders via `searchCriteria`, resolves channel/customer per the sketch (with two confirmed contract corrections — totals are recomputed server-side from `lines`, not passed verbatim; `customerSnapshot` nests under `customer`), dispatches `sales.orders.create`/`customers.people.create` via `commandBus` (now exposed on `AdapterContext`), and tracks external-id mappings for idempotent re-runs. 15 new tests (`__tests__/orders.test.ts`); all 140 tests pass, `build`/`typecheck` clean. Live-validated against the sandbox's 2 sample orders — import, customer dedup, and idempotent skip-on-rerun all confirmed against real data.
