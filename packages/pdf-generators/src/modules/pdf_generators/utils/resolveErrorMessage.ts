...
export async function resolveErrorMessage(response: Response | undefined, t: TranslateFn): Promise<string> {
  const generic = t('pdf_generators.preview.error', 'Failed to generate document.')
  if (!response) return generic
  try {
    const body = await response.clone().json()
    if (body?.error === 'organization_required') {
      return t('pdf_generators.errors.organization_required', 'Select an organization to generate this document.')
    }
