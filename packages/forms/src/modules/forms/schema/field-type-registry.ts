...
// Same boolean payload as YES_NO_TYPE; differs only in widget hint + palette
// presentation. The renderer (registered separately) is a two-button row
// instead of a slider toggle, for forms where the binary choice should read
// as 'pick one of two' rather than 'toggle on/off'.
export const YES_NO_BUTTONS_TYPE: FieldTypeSpec = {
  validator: (value) => (typeof value === 'boolean' ? true : 'Expected true or false.'),
  renderer: null,
  defaultUiSchema: { widget: 'yes_no_buttons' },
  exportAdapter: (value) => (value === true ? 'Yes' : value === false ? 'No' : ''),
  category: 'input',
  icon: 'toggle-right',
  displayNameKey: 'forms.studio.palette.input.yesNoButtons',
}
...
export const V1_FIELD_TYPES = {
  ...
  yes_no: YES_NO_TYPE,
  yes_no_buttons: YES_NO_BUTTONS_TYPE,
  date: DATE_TYPE,
