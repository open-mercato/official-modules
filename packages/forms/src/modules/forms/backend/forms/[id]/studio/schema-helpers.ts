...
import { validateOmTheme } from '../../../../schema/style-validators'
import type { OmFieldStyle, OmSectionStyle, OmTheme } from '../../../../schema/style-extensions'
...
export const SWAP_FAMILIES: Record<string, ReadonlySet<string>> = {
  ...
  boolean: new Set(['boolean', 'yes_no', 'yes_no_buttons']),
  yes_no: new Set(['boolean', 'yes_no', 'yes_no_buttons']),
  yes_no_buttons: new Set(['boolean', 'yes_no', 'yes_no_buttons']),
...
const FIELD_TYPE_TO_DEFAULT_UI: Record<string, Record<string, unknown>> = {
  ...
  yes_no_buttons: { widget: 'yes_no_buttons' },
  date: { widget: 'date' },
...
/**
 * Returns `true` when an `OmTheme` carries no authored member, so the key can
 * be cleared from the persisted schema (keeps the round-trip byte-minimal —
 * R-9 / R-CS-6). An empty theme is equivalent to today's appearance (D4).
 */
function isEmptyTheme(theme: OmTheme | undefined): boolean {
  if (!theme) return true
  return Object.keys(theme).length === 0
}

/**
 * Sets (or clears) the form-wide visual theme (`x-om-theme`, spec D4). An
 * `undefined`/empty theme deletes the key so an unstyled form round-trips
 * byte-identically (R-CS-6). The result is validated through `validateOmTheme`
 * before returning — invalid themes throw `SchemaHelperError`, mirroring the
 * neighbouring form-level setters.
 */
export function setFormTheme(schema: FormSchema, theme: OmTheme | undefined): FormSchema {
  const next = deepClone(schema)
  if (isEmptyTheme(theme)) {
    delete (next as Record<string, unknown>)[OM_ROOT_KEYWORDS.theme]
  } else {
    ;(next as Record<string, unknown>)[OM_ROOT_KEYWORDS.theme] = theme
  }
  validateSchemaExtensions(next)
  return next
}
...
/**
 * Immutably sets (or clears) a single `OmTheme` member, validating the merged
 * theme. Passing `undefined` removes that member; when the resulting theme is
 * empty the whole `x-om-theme` key is cleared.
 */
export function setFormThemeField<Key extends keyof OmTheme>(
  schema: FormSchema,
  key: Key,
  value: OmTheme[Key] | undefined,
): FormSchema {
  const current = (schema as Record<string, unknown>)[OM_ROOT_KEYWORDS.theme] as OmTheme | undefined
  const nextTheme: OmTheme = { ...(current ?? {}) }
  if (value === undefined) {
    delete nextTheme[key]
  } else {
    nextTheme[key] = value
  }
  return setFormTheme(schema, nextTheme)
}

/**
 * Sets (or clears) a section's per-section appearance (`OmSection.style`, spec
 * Step 7). An `undefined`/empty style deletes the member so an unstyled section
 * round-trips byte-minimal (R-CS-6). Mirrors `setSectionColumns` for section
 * lookup and `setFormTheme` for the empty/validate contract: invalid styles
 * throw `SchemaHelperError` via `validateSchemaExtensions`.
 */
...
/**
 * Sets (or clears) a field's per-field appearance (`x-om-style`, spec Step 7).
 * An `undefined`/empty style deletes the key so an unstyled field round-trips
 * byte-minimal (R-CS-6). Mirrors `setFieldGridSpan` for field lookup; invalid
 * styles throw `SchemaHelperError` via `validateSchemaExtensions`.
 */
export function setFieldStyle(
  ...
  if (!style || Object.keys(style).length === 0) {
    delete node[OM_FIELD_KEYWORDS.style]
  } else {
    node[OM_FIELD_KEYWORDS.style] = style
  }
  validateSchemaExtensions(next)
  return next
}
