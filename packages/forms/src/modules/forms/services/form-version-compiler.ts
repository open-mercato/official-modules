...
// ============================================================================
// Read-time style derivation (custom-styling spec 2026-05-22 — D7 / R-CS-6)
// ============================================================================

/**
 * Reads the form-wide `x-om-theme` keyword and returns the typed `OmTheme`
 * when present, otherwise `undefined`.
 *
 * Read-only (D7 / AGENTS.md MUST §12): the persisted schema is never mutated.
 * There are no defaults to apply — absent styling ⇒ `undefined` ⇒ no styling.
 * The matching validator runs defensively; a malformed value (should not occur
 * on a published, pre-validated version) yields `undefined` rather than a throw.
 */
export function resolveFormTheme(schema: Record<string, unknown>): OmTheme | undefined {
  const value = schema[OM_ROOT_KEYWORDS.theme]
  if (value === undefined) return undefined
  if (validateOmTheme(value)) return undefined
  return value as OmTheme
}

/**
 * Reads the per-section `style` member and returns the typed `OmSectionStyle`
 * when present, otherwise `undefined`. Read-only (D7) — never mutates the input
 * section descriptor or the persisted schema. Defensive validator guard.
 */
export function resolveSectionStyle(
  section: OmSection | Record<string, unknown>,
): OmSectionStyle | undefined {
  if (!section || typeof section !== 'object' || Array.isArray(section)) return undefined
  const value = (section as Record<string, unknown>).style
  if (value === undefined) return undefined
  if (validateOmSectionStyle(value)) return undefined
  return value as OmSectionStyle
}

/**
 * Reads the per-field `x-om-style` keyword and returns the typed `OmFieldStyle`
 * when present, otherwise `undefined`. Read-only (D7) — never mutates the field
 * node or the persisted schema. Defensive validator guard.
 */
export function resolveFieldStyle(
  fieldNode: Record<string, unknown>,
): OmFieldStyle | undefined {
  if (!fieldNode || typeof fieldNode !== 'object' || Array.isArray(fieldNode)) return undefined
  const value = fieldNode[OM_FIELD_KEYWORDS.style]
  if (value === undefined) return undefined
  if (validateOmFieldStyle(value)) return undefined
  return value as OmFieldStyle
}
