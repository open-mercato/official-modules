...
import { styleProps } from './style/applyStyle'
...
export function FormRunner(props: FormRunnerProps) {
  ...
  // Form theme apply seam (spec D2 / Step 5): compile the root `x-om-theme`
  // once into scoped DS-var remaps + utility classes. The resulting wrapper is
  // a pure scope/cascade boundary — empty theme ⇒ empty style + no classes, so
  // an unstyled form renders identically to today.
  const formTheme = React.useMemo(
    () => styleProps(compileFormTheme(resolveFormTheme(schema ?? {}))),
    [schema],
  )
  const themeLogo = React.useMemo(() => resolveFormTheme(schema ?? {})?.logo, [schema])
  ...
  // Section apply seam: the current rendered section's `style` member.
  const sectionStyle = styleProps(
    compileSectionStyle(currentSection ? resolveSectionStyle(currentSection) : undefined),
    'flex flex-col gap-4',
  )

  return (
    <div
      className={cn('om-form-theme mx-auto w-full max-w-2xl', formTheme.className)}
    ...
    <article className="flex w-full flex-col gap-6">
      ...
      <section className={sectionStyle.className} style={sectionStyle.style}>
        {sectionFieldKeys.map((fieldKey) => {
          ...
          // Field apply seam: `--om-*` color vars + alignment/labelWeight
          // classes cascade from the wrapper to the renderer's label/inputs
          // (intended v1 behavior). The stable key moves to the wrapper.
          const fieldStyle = styleProps(compileFieldStyle(resolveFieldStyle(node)))
          return (
            <div key={fieldKey} className={fieldStyle.className} style={fieldStyle.style}>
